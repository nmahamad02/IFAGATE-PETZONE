...
export class SupplierDetailComponent {
  ...
  constructor(private route: ActivatedRoute, private dialog: MatDialog, private router: Router, private accountService: AccountsService,private reportsService: ReportsService, private dataSharingService: DataSharingService) { 
    ...
    if (pcode === 'new') {
      ...
      this.accountService.getOpbal(this.currentYear.toString(), pcode).subscribe((res: any) => {
        ...
        this.supForm = new FormGroup({
          ...
          supBranch: new FormControl(res.recordset[0].BRANCH_ID, [ Validators.required]),
          supAccountType: new FormControl(res.recordset[0].ACCOUNT_TYPE_CD, [ Validators.required]),
          supAccountCategory: new FormControl(res.recordset[0].ACCOUNT_CATEGORY_CD, [ Validators.required]),
  ...
  submitForm() {
    ...
    getSupplierSoa(){
      ...
      this.reportsService.getCustomerSoa('S', data.pcode).subscribe((res: any) => {
