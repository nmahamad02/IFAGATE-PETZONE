...
async saveCATOVR() {
 ...
 try {
  ...
  const groups = this.getGroupedCATOVRData();

  groups.forEach(group => {
    // Category header
    exportRows.push({
      'Account Code': group.category,
      'Parent Name': '',
      'Category': '',
      '0 - 30 Days': '',
      '31 - 60 Days': '',
    ...
    group.rows.forEach(row => {
      exportRows.push({
        ...
        '0 - 30 Days': row.Thirty_DAYS ?? 0,
        '31 - 60 Days': row.Sixty_DAYS ?? 0,
        '61 - 90 Days': row.Ninety_DAYS ?? 0,
        '91 - 120 Days': row.OneTwenty_DAYS ?? 0,
        'Above 120 Days': row.ABOVE_120_DAYS ?? 0,
        'Outstanding': row.BALANCE ?? 0,
        'Overdue': row.OVERDUE ?? 0,
        'Period Collection': row.COLLECTED ?? 0,
        'Payment Terms': row.REMARKS ?? ''
      });
    });
    ...
    exportRows.push({
      ...
      'Parent Name': '',
      'Category': '',
      '0 - 30 Days': group.totals.Thirty_DAYS,
      '31 - 60 Days': group.totals.Sixty_DAYS,
      '61 - 90 Days': group.totals.Ninety_DAYS,
      '91 - 120 Days': group.totals.OneTwenty_DAYS,
      'Above 120 Days': group.totals.ABOVE_120_DAYS,
      'Outstanding': group.totals.BALANCE,
      'Overdue': group.totals.OVERDUE,
      'Period Collection': group.totals.COLLECTED,
      'Payment Terms': ''
    });
  ...
  exportRows.push({
    ...
    'Parent Name': '',
    'Category': '',
    '0 - 30 Days': this.getCATOVRTotal('Thirty_DAYS'),
    '31 - 60 Days': this.getCATOVRTotal('Sixty_DAYS'),
    '61 - 90 Days': this.getCATOVRTotal('Ninety_DAYS'),
    '91 - 120 Days': this.getCATOVRTotal('OneTwenty_DAYS'),
    'Above 120 Days': this.getCATOVRTotal('ABOVE_120_DAYS'),
    'Outstanding': this.getCATOVRTotal('BALANCE'),
    'Overdue': this.getCATOVRTotal('OVERDUE'),
    'Period Collection': this.getCATOVRTotal('COLLECTED'),
    'Payment Terms': ''
  });
